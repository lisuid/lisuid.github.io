...
$.ajax({
    ...
    data: {
        key: 'DE7BZ-HAMKA-E2RKJ-CTHJH-2KBAQ-GGBSC',
        output: 'jsonp',
    },
    dataType: 'jsonp',
...
function showWelcome() {
    ...
    let dist = getDistance(28.305516,115.865085, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
    ...
    try {
        ...
        document.getElementById("welcome-info").innerHTML =
            `<span>热烈欢迎来自~</span><br><span><span style="color: var(--icat-welcome-color);font-weight: bold;">${pos}</span> 的喵友</span><br><span class="welcome-message">${posdesc}</span><br>您当前位置距博主约 <b><span style="color: var(--icat-welcome-color);font-weight: bold;">${dist}</span></b> 公里！<br><span>您的IP地址为：${ip}</span><br>${timeChange}`;
    } catch (err) {
